...
def drop(input, p, training=True, inplace=True):
    if p == 0:
        return input
    if not training:
        return _back_dropout(input, p, training, inplace)
    assert len(input.shape) == 3
    if len(input.shape) == 3:
        zero_mask = input == 0
        input[:, :, ::2] = p
        input.masked_fill_(zero_mask, 0)
    return input
# F.dropout(input, self.p, self.training, self.inplace)
_back_dropout = F.dropout


def drop_noaction(input, p, training=True, inplace=True):
    return input


def replace_dropout(nodrop=False):
    if nodrop:
        F.dropout = drop_noaction
    else:
        F.dropout = drop


def unreplace_dropout():
    F.dropout = _back_dropout
...
def get_testdata_trainer(dir=None, rm_exp=False):
    dir = dir or '/tmp/exp-en-de'
    return build_trainer(_testdata_argline.format(dir), rm_exp=rm_exp)[0]


def assert_tensor_eq(a, b, thr=0.0, max_thr=0.0):
    ...
    a_b = (a - b).abs()
    assert a_b.sum().item() <= thr and a_b.max().item() <= max_thr, (a_b.sum().item(), a_b.max().item(), a, b)
...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    """
    import torch
    from torch.nn.functional import linear, has_torch_function, handle_torch_function, softmax, dropout, pad, Tensor
    res = {}
    ...
    res['qkt'] = attn_output_weights

    assert list(attn_output_weights.size()) == [bsz * num_heads, tgt_len, src_len]

    if attn_mask is not None:
        attn_output_weights += attn_mask
    res['qkt attn mask'] = attn_output_weights
    ...
    if key_padding_mask is not None:
        ...
        attn_output_weights = attn_output_weights.view(bsz * num_heads, tgt_len, src_len)
    res['qkt pad mask'] = attn_output_weights

    attn_output_weights = softmax(
        attn_output_weights, dim=-1)
    res['softmax'] = attn_output_weights

    attn_output_weights = dropout(attn_output_weights, p=dropout_p, training=training)
    res['dropout'] = attn_output_weights
    ...
    res['linear'] = attn_output
    res['weights'] = attn_output_weights
