...
def tfrecord_iterator(
    ...
    if use_mmap:
        print('use mmap')
        assert compression_type is None, 'use_mmap only supports raw .tfrecord files'
        ori_file = file 
        file = mmap.mmap(file.fileno(), length=0, access=mmap.ACCESS_READ)
        file.__enter__()
    ...
    def read_records(start_offset=None, end_offset=None):
        ...
        while file.tell() < end_offset:
            ...
            if read_into(file, length_bytes) != 8:
                raise RuntimeError("Failed to read the record size.")
            if read_into(file, crc_bytes) != 4:
                raise RuntimeError("Failed to read the start token.")
            ...
            if read_into(file, datum_bytes_view) != length:
                raise RuntimeError("Failed to read the record.")
            if read_into(file, crc_bytes) != 4:
                raise RuntimeError("Failed to read the end token.")
...
def process_feature(feature: example_pb2.Feature,
    ...
    if typename is not None:
        ...
        if tf_typename != inferred_typename:
            ...
            raise TypeError(f"Incompatible type '{typename}' for `{key}` "
                        f"(should be '{reversed_mapping[inferred_typename]}').")
    # return list instead of np.array
    if inferred_typename == "bytes_list":
        # value = bytes(value[0])
        value = [bytes(v).decode('utf8') for v in value]  # convert to utf8 string
    else:
        value = list(value)
    # elif inferred_typename == "float_list":
    #     value = np.array(value, dtype=np.float32)
    # elif inferred_typename == "int64_list":
    #     value = np.array(value, dtype=np.int64)
    return (inferred_typename, value)
